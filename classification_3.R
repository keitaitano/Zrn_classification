...
df_new <-
  ...
  select(Label3,P, Sc, Y:Nb, La:Lu, Hf, Th, U, index)
element_order <- c("P", "Sc", "Y","Nb", "La", "Ce", "Pr", "Nd", "Sm", "Eu", "Gd", "Tb", "Dy", "Ho", "Er","Tm", "Yb","Lu","Hf","Th","U")
...
df_clr <- 
  ...
  select(-Label3, -index) %>%
  clr(.) %>% 
  data.frame(index=df_screened3$index, Label3=df_screened3$Label3, .)
...
## sample size
tmp<- rbind(table(df$Label3), table(df_clr$Label3)) 
rownames(tmp) <- c("Raw data", "Screened")
write.csv(tmp, "Output/classification_3/data.csv")
## correlation plot
png("Output/classification_3/correlation.png")
...
dev.off()
## PCA contribution
sink("Output/classification_3/pca_contribution.txt")
summary(pca)
closeAllConnections()
## PCA loading
pca$rotation %>% data.frame(.) %>% mutate(element=rownames(.)) %>% 
  ...
  coord_fixed(ratio=1) +
  theme_bw()
ggsave("Output/classification_3/PC1_PC2.png")
pca$rotation %>% data.frame(.) %>% mutate(element=rownames(.)) %>% 
  ...
  theme_bw()
ggsave("Output/classification_3/PC1_PC3.png")
pca$rotation %>% data.frame(.) %>% mutate(element=rownames(.)) %>% 
  ...
  coord_fixed(ratio=1) +
  theme_bw()
ggsave("Output/classification_3/PC1_PC4.png")
...
df_cv<-c()
df_cv <- vfold_cv(df_clr, strata = "Label3", v = k_fold) 
df_cv_class <-  df_clr$Label3 %>% table() %>%　t()
set.seed(32)
for (i in 1: k_fold){
  df_cv$splits[[i]] %>% assessment() %>% select(Label3) %>% table() %>% t() -> tmp
  df_cv_class <- rbind(df_cv_class, tmp)
}
rownames(df_cv_class) <- c("Pre-splitting", 1:10)
## stratified k-fold cross-validation (PCA data)
df_pca_cv <- vfold_cv(df_pca, strata = "Label3", v = k_fold)
df_pca_cv_class <-  df_pca$Label3 %>% table() %>%　t()
set.seed(32)
for (i in 1: k_fold){
  df_pca_cv$splits[[i]] %>% assessment() %>% select(Label3) %>% table() %>% t() -> tmp
...
for (i in 1:k_fold) {
  ...
  Sys.sleep(1 / 100)
  ### dataset
  df_cv$splits[[i]] %>% analysis() %>% select(-index) -> df_train
  df_cv$splits[[i]] %>% assessment() %>% select(-index) -> df_test
  ### model training
  set.seed(32)
  model_cart <- train(data = df_train, Label3 ~ ., method = "rpart", trControl = trainControl("cv", number = 10))
  ### results of CART
  # fancyRpartPlot(model_cart$finalModel)
  hp_tmp <- model_cart$bestTune  %>% unlist()
  p_train_cart_tmp <- confusionMatrix(model_cart) %>% .$table
  p_test_cart_tmp <- model_cart %>% predict(df_test) %>% confusionMatrix(., df_test$Label3) %>% .$table
...
for (i in 1:k_fold) {
  ...
  p_test_rf_tmp <- model_rf %>% predict(df_test) %>% confusionMatrix(., df_test$Label3) %>% .$table
...
for (i in 1:k_fold) {
  pb$tick()
  df_pca_cv$splits[[i]] %>% analysis() %>% select(-index) -> df_train
  df_pca_cv$splits[[i]] %>% assessment() %>% select(-index) -> df_test
  ### model training
  set.seed(32)
  model_svm <- train(
    Label3 ~.,
  ...
  p_test_svm_tmp <- model_svm %>% predict(df_test) %>% confusionMatrix(., df_test$Label3) %>% .$table
...
write.csv(predict_peformances, "Output/classification_3/predict_all.csv")

##confusion matrix (training data)
file_path <- "Output/classification_3/ConfusionMatrix_train.txt"
sink(file_path)
...
file_path <- "Output/classification_3/ConfusionMatrix_test.txt"
sink(file_path)
"Confusion matrix: CART"
sink(file_path,  append = TRUE)
p_test_cart
...
file_path <- "Output/classification_3/prediction_class_train.txt"
sink(file_path)
"CART" 
sink(file_path,  append = TRUE)
performance_cart_train_class
...
file_path <- "Output/classification_3/prediction_class_test.txt"
sink(file_path)
"CART" 
sink(file_path,  append = TRUE)
performance_cart_test_class
...
## hyper parameter
file_path <- "Output/classification_3/hyperperameter.txt"
sink(file_path)
...
## variable importance
file_path <- "Output/classification_3/variable importance.txt"
sink(file_path)
"CART" 
sink(file_path,  append = TRUE)
